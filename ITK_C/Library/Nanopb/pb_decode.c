...
/* Impinj Custom  Extensions */

/* Given a populated dest_struct and a field description, count
 * up the number of optional (has_*) fields
 */
int pb_get_optional_count(const pb_field_t fields[], void *dest_struct)
{
    int count = 0;
    pb_field_iterator_t iter;
    pb_field_init(&iter, fields, dest_struct);
    ...
    do
    {
        ...
        if (PB_HTYPE(iter.current->type) == PB_HTYPE_OPTIONAL)
        {
            /* pSize is also the has_* field for optional tags */
            if(*(bool*)iter.pSize == true)
            {
                count++;
            }
        }
